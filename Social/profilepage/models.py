...
class FriendsManager(models.Manager):
    # 프로필의 주인이 팔로우 하는 사람들(followiing)
    def get_following_users(self, profile_user):
        qs = Friends.objects.filter(sender_idx=profile_user)
        # qs = Friends.objects.filter(sender_idx=profile_user).values_list('receiver_idx', flat=True)
        return qs
    ...
    def get_followers(self, profile_user):
        qs = Friends.objects.filter(receiver_idx=profile_user)
        return qs
...
class UserDisplaySmall:
    ...
    def setProfilePic(self):
        try:
            dic = json.loads(self.user.profile)
            self.profile_pic = dic["profile_pic"]
        except:
            pass
    ...
    def setBackgroundPic(self):
        try:
            dic = json.loads(self.user.profile)
            self.background_pic = dic["background_pic"]
        except:
            pass
...
class ProfileCommentsDisplayModel:
    track = Tracks()
    user = Users()
    comment = Comments()
    post = Posts()
    ...
    def __init__(self, user, post, comment):
        self.user = user
        self.post = post
        self.track = post.track_idx
        self.comment = comment
        #self.setComments()

    def setComment(self):
        self.comment = Comments.objects.get(users_idx=self.user, posts_idx=self.post)
